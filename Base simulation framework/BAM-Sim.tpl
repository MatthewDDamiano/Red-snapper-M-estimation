...
init_int e_ckmr;                                      // final ckmr sample
...
init_vector set_fecpar_batches(1,nages); //added back in 3-26-25
...
// Natural mortality
init_vector M_len(1,nages); // length vector for Lorenzen M fcn (2022)
init_number M_lor_scale; // scaling value (fixed) to get unscale M at age 
...
//--------------Growth--------------------------------------------------------------------------- 
  ...
  init_bounded_number M_b(M_b_LO,M_b_HI,M_b_PH);
  number M_scale; //Scalar on M in arithmetic space (back transformed from logit); constrained to (0,2)
  vector M_lorenzen(1,nages);
  vector M_unscaled(1,nages);
...
//-----Population-----------------------------------------------------------------------------------
  ...
  vector ckmr_fec(1,nages);                 //vector used to compute the measure of fecundity needed for ckmr functions: maturity*fecundity*batches
...
////Close kin mark recapture
  number phsp;                  //intermediate probability of being a half-sibling pair
  number ppop;                  //intermediate probability of being a parent-offspring pair
  number PHSP;                  //total probability of being an hsp
  number PPOP;                  //total probability of being a pop
  vector PHSP_vec(f_ckmr,e_ckmr);
  vector PPOP_vec(f_ckmr,e_ckmr);
  number nll_ckmr;              //ckmr LL
...
FUNCTION get_hsp_prob // calculate reproductive output and probability of being an hsp (ckmr)
...
FUNCTION get_pop_prob // calculate reproductive output and probablity of being a pop (ckmr)
  for(int i=f_ckmr;i<e_ckmr;i++)
  {
    ppop = 0.0;
