...
ObsModel <- function(L1, L2, D, survey, L1.age, L2.age, D.len, survey.age, cv.L1, cv.L2, cv.D, cv.survey,
  ...
  # Matt V's approach
  ln.L1 <- rnorm(nobs.L1, mean = -sd.L1^2/2, sd = sd.L1)
  ln.L2 <- rnorm(nobs.L2, mean = -sd.L2^2/2, sd = sd.L2)
  ln.D <- rnorm(nobs.D, mean = -sd.L2^2/2, sd = sd.D)
  ln.survey <- rnorm(nobs.survey, mean = -sd.survey^2/2, sd = sd.survey)
