...
profile.vals1 <- seq(-3, 3, by = 0.5)
#
vals <- 2 * exp(profile.vals1) / (1 + exp(profile.vals1))

# par.name <- "M b"
# profile.vals <- seq(-2.0,-0.1, by = 0.2)
# 
# # profile.vals <- seq(0.5,1.0, by = 0.05)
# vals <- profile.vals
...
spp1 <- dget(paste(folder, iter, "/BAM-Sim-1.rdat", sep = ""))
spp2 <- dget(paste(folder, iter, "/BAM-Sim-2.rdat", sep = ""))
spp3 <- dget(paste(folder, iter, "/BAM-Sim-3.rdat", sep = ""))
spp4 <- dget(paste(folder, iter, "/BAM-Sim-4.rdat", sep = ""))
spp5 <- dget(paste(folder, iter, "/BAM-Sim-5.rdat", sep = ""))
spp6 <- dget(paste(folder, iter, "/BAM-Sim-6.rdat", sep = ""))
spp7 <- dget(paste(folder, iter, "/BAM-Sim-7.rdat", sep = ""))
spp8 <- dget(paste(folder, iter, "/BAM-Sim-8.rdat", sep = ""))
spp9 <- dget(paste(folder, iter, "/BAM-Sim-9.rdat", sep = ""))
spp10 <- dget(paste(folder, iter, "/BAM-Sim-10.rdat", sep = ""))
spp11 <- dget(paste(folder, iter, "/BAM-Sim-11.rdat", sep = ""))
...
lk.tot <- out2[, 2]
lk.SR <- out2[, 3]
lk.cpue <- out2[, 4] # rowSums(out2[,4:6])
# lk.cpue.fleet1 <- out2[, 5]
lk.len <- (out2[, 8])
lk.age <- rowSums(out2[, 9:11])
lk.age.survey1 <- out2[, 9]
lk.age.fleet1 <- out2[, 10]
lk.age.fleet2 <- out2[, 11]
# 