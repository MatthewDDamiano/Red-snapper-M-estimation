...
# Last updated: 24 Sept 2026 by MDD and KWS
...
#                     includes prototype CKMR integration (on BAM side) with limited data generation - not used in this model
#####################################################################################
...
# 1 prefix denotes a core simulation model file, 0 prefix denotes specific function files
...
source("0funcCKMR.R") # currently reads in dummy data for "observed" half-sibling and parent-offspring
...
styr.survey <- 1 # year to start survey (for index and age comps)
styr.ac1 <- 1 # year to start age comps of the fleet1 landings
styr.ac2 <- 1 # year to start age comps of the fleet2 landings
styr.d <- 1 # year to start length comps for discards (fleet 3)
...
# uncomment to use model that starts in 1976
# nyr <- 45
# F1.mu <- F1.mu[26:70]
# F2.mu <- F2.mu[26:70]
# F3.mu <- F3.mu[26:70]
...
sloped.sel3 <- 0.0001 # Slope of selectivity for dead discards (fleet 3) descending limb
...
# plot(selex1) # check - fleet 1 is recreational
selex2 <- logistic(ages, slope.sel2, A50.sel2) # selectivity of fleet 2
# plot(selex2) # check - fleet 2 is commercial
selex3 <- double.logistic(ages, slopea.sel3, A50a.sel3, sloped.sel3, A50d.sel3) # selectivity of fleet 3
# plot(selex3) # check - default is treat discard selex as mirror of rec fleet (fleet 1)
...
#####################################################################################
# Life-history parms
SR.switch <- 3 # SR function switch (1=BH, 2=Ricker, 3=Null) # Null is default for M sim study
...
R0 <- 383000 # Average annual unfished recruitment (scales the popn)
h <- 0.999 # Steepness of the spawner-recruit relationship; not used in Null model
logR.sd <- 0.51 # Standard deviation of log recruitment
...
K <- 0.24 # Growth coefficient
a0 <- -0.33 # Theoretical age at size 0
cv.laa <- 0.1 # CV of length at age; only needed if fitting length comps
...
# batches at age for batch fecundity
batch <- c(22, 45, 63, 77, 88, 97, 104, 109, 114, 117, 120, 122, 123,
...
len.cm <- len / 10 # scales length to cm (default is mm)
...
Mlorenzen <- M.lorenzen(length = len.cm, a = 0.0, b = M.b) # parameter values from Lorenzen et al. 2022 paper

M.lor.scale <- 0.287 # 0.287 scales similarly to red snapper

M.age.unscaled <- M.lor.scale * Mlorenzen / max(Mlorenzen) 
M.age.unscaled
mean(M.age.unscaled)
...
reprod <- proportion.female * mat.age * fecundity * batch
...
logR.resid <- rnorm(nyr, mean = -logR.sd^2/2, sd = logR.sd)
...
lenprob <- lenprob.fcn(len.mu = len, len.cv = cv.laa, len.bins = lenbins, len.binw = binw) # see 0funcLenProb.R for function code
...
# For reference, the following parameters were estimated in SEDAR 73:
# "cvlen, "len_cv_val," "len_cv_val_L," ""len_cv_val_20,"
# log_R0, rec_sigma, log_rec_dev, dirichlet overdispersion pars, 
# selectivity parameters, qs, average Fs by fleet, Fdevs by fleet, and F_init
...
parms.Linf <- c(Linf, 500.0, 1000.0, -4, Linf, -0.25, 1)
...
parms.M.b <- c(M.b, -2.0, -0.1, -4, M.b, -0.25, 1) # what prior var, pdf should this par have? 
dat.parms <- rbind(dat.parms, parms.M.b)
...
fec.parms <- list(batch = batch, fec.a = fec.a, fec.b = fec.b, fec.c = fec.c, fec.thresh = fec.thresh, fec.min = fec.min)
...
BAM.write.dat(
  ...
  len.cm = len.cm, M.lor.scale = M.lor.scale
)
...
# #####################################################################################
# library(FishGraph)
# ptype <- "pdf" # (no quotes) for no plots saved; other options: "pdf", "wmf", "eps"
...
# #####################################################################################
 ...
 # )
 # RE.M.scale <- (bam$parms$M.scale - M.mult) / M.mult
 # RE.M.b <- (bam$parms$M.b - M.b) / M.b
 ...
 #   RE <- c(RE.h, RE.R0, RE.M.scale, RE.M.b, RE.MSY, RE.SSBmsy, RE.Fmsy)
 #   RE.names <- c("h", "R0", "M scale par", "M b par", "MSY", "SSBmsy", "Fmsy")
 # } else if (SR.switch == 3) {
 #   RE <- c(RE.R0, RE.M.scale, RE.M.b, RE.Fproxy, RE.SSBFproxy)
 #   RE.names <- c("R0", "M scale par", "M b par", "Fproxy", "SSBFproxy")
 # }
