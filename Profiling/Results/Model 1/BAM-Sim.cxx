...
open_r_info_list("info", true);
	...
	wrt_r_item("base.run", "BAM-Sim.tpl");  	
...
open_r_info_list("parms", false); 
    ...
    wrt_r_item("F.prop.fleet2", F_fleet2_prop);		
	wrt_r_item("F.prop.fleet3", F_fleet3_prop);	
	wrt_r_item("F.init.scale", F_init);
	wrt_r_item("log.F.avg.fleet1", log_avg_F_fleet1);
	wrt_r_item("log.F.avg.fleet2", log_avg_F_fleet2);
        wrt_r_item("log.F.avg.fleet3", log_avg_F_fleet3);
	...
	wrt_r_item("biascorr", BiasCor);		
	wrt_r_item("M.scale", M_scale); //M scalar in arithmetic space
    wrt_r_item("M.b", M_b); // slope/scaling parameter in Lorenzen M fcn (2022)
	...
	wrt_r_item("Fproxy", Fproxy_out); 
	wrt_r_item("SSB.Fproxy", SSB_Fproxy_out); 
	wrt_r_item("msst.Fproxy", smsy2msst*SSB_Fproxy_out);
close_r_info_list();
...
open_r_info_list("spr.brps", false);
	...
	wrt_r_item("F50", F50_out);
	wrt_r_item("Fproxy", Fproxy_out); 
	wrt_r_item("SSB.Fproxy", SSB_Fproxy_out);
    wrt_r_item("msst.Fproxy", smsy2msst*SSB_Fproxy_out);   
    wrt_r_item("B.Fproxy", B_Fproxy_out);
	wrt_r_item("R.Fproxy", R_Fproxy_out);
    wrt_r_item("L.Fproxy.knum", L_Fproxy_knum_out);
    wrt_r_item("D.Fproxy.knum", D_Fproxy_knum_out);
    wrt_r_item("L.Fproxy.mt", L_Fproxy_mt_out);
    wrt_r_item("D.Fproxy.mt", D_Fproxy_mt_out);
    wrt_r_item("Fend.Fproxy.mean", FdFproxy_end_mean);
    wrt_r_item("SSBend.SSBFproxy", SdSSB_Fproxy_end);
	wrt_r_item("SSBend.MSSTFproxy", Sdmsst_Fproxy_end);	
close_r_info_list();
...
open_r_df("parm.cons",1,8,2);
	...
	wrt_r_df_col("M.scale.logit", M_logit_out);
	wrt_r_df_col("M.b", M_b_out);
	...
	wrt_r_df_col("log_dm_fleet2_ac",log_dm_fleet2_ac_out);
	wrt_r_df_col("log_dm_fleet3_lc",log_dm_fleet3_lc_out);
    ...
    wrt_r_df_col("selpar_slope_fleet1_B1",selpar_slope_fleet1_B1_out);
 	wrt_r_df_col("selpar_A50_fleet2_B1",selpar_A50_fleet2_B1_out);
    wrt_r_df_col("selpar_slope_fleet2_B1",selpar_slope_fleet2_B1_out);

       wrt_r_df_col("selpar_A50_asc_fleet3_B1",selpar_A50_asc_fleet3_B1_out);
    wrt_r_df_col("selpar_slope_asc_fleet3_B1",selpar_slope_asc_fleet3_B1_out);
 	wrt_r_df_col("selpar_A50_dsc_fleet3_B1",selpar_A50_dsc_fleet3_B1_out);
    wrt_r_df_col("selpar_slope_dsc_fleet3_B1",selpar_slope_dsc_fleet3_B1_out);
	...
	wrt_r_df_col("log_avg_F_fleet2",log_avg_F_fleet2_out);
    wrt_r_df_col("log_avg_F_fleet3",log_avg_F_fleet3_out);
...
open_r_df("parm.tvec", styr, endyr, 2);
	...
	wrt_r_df_col("log.F.dev.fleet2", log_F_dev_fleet2_out);
        wrt_r_df_col("log.F.dev.fleet3", log_F_dev_fleet3_out);
close_r_df();
...
open_r_df("parm.tvec.cons",1,3,2);
	...
	wrt_r_df_col("log.F.dev.fleet2",set_log_F_dev_fleet2);
        wrt_r_df_col("log.F.dev.fleet3",set_log_F_dev_fleet3);
close_r_df();
...
open_r_vector("sdnr"); 
	...
	wrt_r_item("sdnr.agec.fleet2", sdnr_ac_fleet2); 	
close_r_vector();    
...
open_r_vector("like"); 
	...
	wrt_r_item("lk.L.fleet2", f_fleet2_L);
    wrt_r_item("lk.D.fleet3", f_fleet3_D);
    wrt_r_item("lk.U.survey1", f_survey1_cpue);    
    wrt_r_item("lk.agec.survey1", f_survey1_agec);	
    wrt_r_item("lk.agec.fleet1", f_fleet1_agec);
    wrt_r_item("lk.agec.fleet2", f_fleet2_agec);	
    wrt_r_item("lk.lenc.fleet3", f_fleet3_lenc);
	...
	wrt_r_item("w.agec.fleet2", w_ac_fleet2);	
...
close_r_vector();
    open_r_matrix("lenprob");
    wrt_r_matrix(lenprob, 2, 2);
    ...
    open_r_matrix("D.age.pred.num");
    wrt_r_matrix(D_total_num, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);   
    close_r_matrix();
    
    open_r_matrix("D.age.pred.mt");
    wrt_r_matrix(D_total_mt, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);   
    close_r_matrix();
...
open_r_list("sel.age");
    ...
    open_r_matrix("sel.m.fleet2");
    wrt_r_matrix(sel_fleet2, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);   
    close_r_matrix();

    open_r_matrix("sel.m.fleet3");
    wrt_r_matrix(sel_fleet3, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);   
    close_r_matrix(); 
...
open_r_list("comp.mats");
    ...
    open_r_matrix("acomp.fleet2.ob");		
    wrt_r_matrix(obs_fleet2_agec, 2, 2);
    wrt_r_namevector(yrs_fleet2_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
    
    open_r_matrix("acomp.fleet2.pr");		
    wrt_r_matrix(pred_fleet2_agec, 2, 2);
    wrt_r_namevector(yrs_fleet2_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();  

    open_r_matrix("lcomp.fleet3.ob");
    wrt_r_matrix(obs_fleet3_lenc, 2, 2);
    wrt_r_namevector(yrs_fleet3_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();

    open_r_matrix("lcomp.fleet3.pr");
    wrt_r_matrix(pred_fleet3_lenc, 2, 2);
    wrt_r_namevector(yrs_fleet3_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();
...
open_r_df("t.series", styr, (endyr+1), 2);
	...
	wrt_r_df_col("F.Fproxy.ratio", FdFproxy);	//*.ratio extension is for FishGraph
	wrt_r_df_col("F.full", Fapex);		
	wrt_r_df_col("F.fleet1", F_fleet1_out);
	wrt_r_df_col("logF1.dev", log_F1_dev_output);
	wrt_r_df_col("F.fleet2", F_fleet2_out);
	wrt_r_df_col("logF2.dev", log_F2_dev_output);
        wrt_r_df_col("F.fleet3", F_fleet3_out);
        wrt_r_df_col("logF3.dev", log_F3_dev_output);
	...
	wrt_r_df_col("SSB.SSBFproxy", SdSSB_Fproxy);
    wrt_r_df_col("SSB.msstFproxy", Sdmsst_Fproxy);
    wrt_r_df_col("B", totB);
    ...
    wrt_r_df_col("total.D.mt", D_total_mt_yr);
    ...
    wrt_r_df_col("L.fleet2.ob", obs_fleet2_L);			
    wrt_r_df_col("L.fleet2.pr", pred_fleet2_L_mt);  		
    wrt_r_df_col("cv.L.fleet2", fleet2_L_cv);

    wrt_r_df_col("L.fleet3.ob", obs_fleet3_D);			
    wrt_r_df_col("L.fleet3.pr", pred_fleet3_D_mt);  		
    wrt_r_df_col("cv.L.fleet3", fleet3_D_cv);
	
    //comp sample sizes      
    wrt_r_df_col("acomp.survey1.n", nsamp_survey1_agec_allyr);   
    wrt_r_df_col("acomp.fleet1.n", nsamp_fleet1_agec_allyr);  
    wrt_r_df_col("acomp.fleet2.n", nsamp_fleet2_agec_allyr);  	
	wrt_r_df_col("lcomp.fleet3.n", nsamp_fleet3_lenc_allyr);      
	
    wrt_r_df_col("acomp.survey1.nfish", nfish_survey1_agec_allyr); 
    wrt_r_df_col("acomp.fleet1.nfish", nfish_fleet1_agec_allyr);  
    wrt_r_df_col("acomp.fleet2.nfish", nfish_fleet2_agec_allyr);  	
	wrt_r_df_col("lcomp.fleet3.nfish", nfish_fleet3_lenc_allyr); 
	...
	wrt_r_df_col("acomp.fleet2.neff", (1+nsamp_fleet2_agec_allyr*exp(log_dm_fleet2_ac_out(8)))/(1+exp(log_dm_fleet2_ac_out(8))) ); 
    wrt_r_df_col("lcomp.fleet3.neff", (1+nsamp_fleet3_lenc_allyr*exp(log_dm_fleet3_lc_out(8)))/(1+exp(log_dm_fleet3_lc_out(8))) );
close_r_df();
...
open_r_df("LD.pr.tseries", styr, endyr, 2);
    ...
    wrt_r_df_col("L.fleet2.mt", pred_fleet2_L_mt);   
    wrt_r_df_col("L.fleet2.knum", pred_fleet2_L_knum);

	wrt_r_df_col("D.fleet3.mt", pred_fleet3_D_mt);   
    wrt_r_df_col("D.fleet3.knum", pred_fleet3_D_knum);
...
open_r_df("a.series", 1, nages, 2);
	...
	wrt_r_df_col("wgt.wgted.L.mt", wgt_wgted_L_mt);
        wrt_r_df_col("wgt.wgted.D.mt", wgt_wgted_D_mt);
...
open_r_list("CLD.est.mats");
    ...
    open_r_matrix("Lw.fleet2");			
       wrt_r_matrix(L_fleet2_mt, 1,1);		
    close_r_matrix();
    
    open_r_matrix("Ln.fleet2");			
       wrt_r_matrix(L_fleet2_num, 1,1);
    close_r_matrix();
    ...
    open_r_matrix("Ln.fleet3");
        wrt_r_matrix(D_fleet3_num, 1,1);
    close_r_matrix();

    open_r_matrix("Dw.total");
        wrt_r_matrix(D_total_mt, 1,1);
    close_r_matrix();
...
open_r_df("projection", styr_proj, endyr_proj, 2);
	...
	wrt_r_df_col("D.mt.proj", D_mt_proj);
 close_r_df();
